...
fetch("../Data/giftcode.json").then((res) => {
    return res.json();
}).then((giftcodeData) => {
...
function GetGiftcode() {
    ...
    if (valid) {
        ...
        const lastThreeEntries = historyData.filter(entry => entry.mssv === mssv).slice(0, 2);
        const activateInsurance = lastThreeEntries.length === 2 && lastThreeEntries.every(entry => entry.reward === "Chúc bạn may mắn lần sau");
        let insurance = false;
